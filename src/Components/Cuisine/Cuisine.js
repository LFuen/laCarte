...
class Cuisine extends Component {
  ...
  render() {
    ...
    const cuisineOrg =
      cuisines.length > 0
        ? cuisines.find((cuisine) => cuisine.origin === originParameter)
        : "";
    const cuisineMeals = meals.filter(
      (meal) => meal.cuisine_id === cuisineOrg.id
    );

    if (cuisineOrg && cuisineMeals.length > 0) {
      return (
        <div className="dishes">
          <h1 className="cuisineName" key={cuisineOrg}>
            {cuisineOrg.origin} Dishes
          </h1>
